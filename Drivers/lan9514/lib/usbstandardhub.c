...
boolean USBStandardHubEnumeratePorts (TUSBStandardHub *pThis)
{
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++)
	{
		...
		assert (pNames != 0);
                ...
                #ifdef TONY
                printHex("STD HUB: Port: ", nPort+1, 0xFFFFFFFF); 
                println("STD HUB: Device:", 0xFFFFFFFF); 
                println(StringGet(pNames), 0xFFFFFFFF); 
                #else
		...
		if (pChild != 0)
		{
			...
			if (!(*pThis->m_pDevice[nPort]->Configure) (pThis->m_pDevice[nPort]))
			{
				...
				continue;
                        ...
                        #ifdef TONY
                        printHex("STDHUB config!: Port: ", nPort+1, 0xFFFFFFFF); 
                        #else
