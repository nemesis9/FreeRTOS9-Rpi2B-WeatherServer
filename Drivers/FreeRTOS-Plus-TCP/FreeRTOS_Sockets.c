...
#define TBDBG( msg ) println(msg, MAG_TEXT);
#define TBCLIDBG( msg ) println(msg, BLUE_TEXT);
#define TBCLIHDBG( msg, val ) printHex(msg, (unsigned int)val, BLUE_TEXT);
#define TBDBGERR( msg ) println(msg, RED_TEXT);
#define TBHDBG( msg, val ) printHex(msg, (unsigned int)val, MAG_TEXT);
#else
#define TBDBG( msg )
#define TBDBGERR( msg )
			...
			#if( ipconfigSUPPORT_SIGNALS != 0 )
			{
				if( ( xResult & eSELECT_INTR ) != 0u )
				{
					...
					//FreeRTOS_debug_printf( ( "FreeRTOS_select: interrupted\n" ) );
                                        TBDBG("FreeRTOS_select: interrupted")
					break;
				}
			}
	...
	FreeRTOS_Socket_t *xReturn;
		...
		if( xSendEventStructToIPTask( &xSelectEvent, ( TickType_t ) portMAX_DELAY ) == pdFAIL )
		{
			...
			//FreeRTOS_debug_printf( ( "prvFindSelectedSocket: failed\n" ) );
                        TBDBGERR("Failed to wake up IP task")
			xReturn = NULL;
		}
		else
		{
...
EventBits_t xEventBits = ( EventBits_t ) 0;
	...
	if( lPacketCount != 0 )
	{
		...
		if( ( xFlags & FREERTOS_ZERO_COPY ) == 0 )
		{
			...
			if( ( xFlags & FREERTOS_MSG_PEEK ) == 0 )
			{
                                TBDBG("Sock proc recvfrom: vRelease")
			...
			( FreeRTOS_bind( xSocket, NULL, 0u ) == 0 ) )
		{
			...
			if( pxNetworkBuffer != NULL )
			{
				...
				else
				{
					...
					if( ( xFlags & FREERTOS_ZERO_COPY ) == 0 )
					{
                                                TBDBG("Sock proc sendto: vRelease")
						vReleaseNetworkBufferAndDescriptor( pxNetworkBuffer );
					}
...
BaseType_t xReturn = 0;
	...
	else if( socketSOCKET_IS_BOUND( pxSocket) != pdFALSE )
	{
		...
		//FreeRTOS_debug_printf( ( "vSocketBind: Socket already bound to %d\n", pxSocket->usLocalPort ) );
                TBHDBG("bind: already bound to: ", pxSocket->usLocalPort)
		xReturn = -pdFREERTOS_ERRNO_EINVAL;
	}
	else
	{
		...
		if( xSendEventStructToIPTask( &xBindEvent, ( TickType_t ) portMAX_DELAY ) == pdFAIL )
		{
			...
			//FreeRTOS_debug_printf( ( "FreeRTOS_bind: send event failed\n" ) );
                        TBDBGERR("bind send event failed")
	...
	if( pxAddress != NULL )
	{
			...
			( pxListFindListItemWithValue( pxSocketList, ( TickType_t ) pxAddress->sin_port ) != NULL ) )
		{
			...
			//	FreeRTOS_ntohs( pxAddress->sin_port ) ) );
                        TBDBGERR("Socket already in use")
...
/*-----------------------------------------------------------*/
	...
	IPStackEvent_t xAskEvent;
        ...
        //const portTickType xDelayms = pdMS_TO_TICKS( 20000UL );
		...
		else
		{
			...
			for( ; ; )
			{
				...
				xTaskResumeAll();
                                ...
                                TBCLIDBG("Checking pxClient not NULL")
				if( pxClientSocket != NULL )
				{
					if( pxAddress != NULL )
					{
				...
				if( pxClientSocket != NULL )
				{
                                        ...
                                        TBCLIDBG("client socket not null: breaking")
					break;
				}
				...
				/* Go sleeping until we get any down-stream event */
                                TBCLIDBG("sleeping for remaining time")
                                TBCLIHDBG("rem time: ", xRemainingTime)

                                //This is not the right code, but will work, and I tried it just to show that the rest of the code works
                                //vTaskDelay(xDelayms);

                                //This is supposed to be the right code, but will block forever, even after TCP client connects
				xEventGroupWaitBits( pxSocket->xEventGroup, eSOCKET_ACCEPT, pdTRUE /*xClearOnExit*/, pdFALSE /*xWaitAllBits*/, xRemainingTime );
                                TBCLIDBG("done sleeping for remaining time")
			}
		}
			...
			if( pxSocket->usLocalPort == ( uint16_t ) uxLocalPort )
			{
				...
				else if( ( pxSocket->u.xTCP.usRemotePort == ( uint16_t ) uxRemotePort ) && ( pxSocket->u.xTCP.ulRemoteIP == ulRemoteIP ) )
				{
					...
					break;
                                    ...
                                    TBDBG("TCP socket lookup: no match")
                                }
			}
		}
		if( pxResult == NULL )
