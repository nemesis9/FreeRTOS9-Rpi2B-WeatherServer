...
#ifdef TONY
#include <video.h>
#define TBDBG( msg ) println(msg, VIOLET_TEXT);
#define TBHDBG( msg, val ) printHex(msg, (unsigned int)val, VIOLET_TEXT);
#else
#define TBDBG( msg )
		...
		switch( xReceivedEvent.eEventType )
		{
			...
			case eTCPAcceptEvent:
				...
				#if( ipconfigUSE_TCP == 1 )
				{
					...
					{
                                                TBHDBG("Set socket accept for: ", pxSocket)
	...
	switch( eReturned )
	{
		...
		default :
			...
			just be	released back to the list of free buffers. */
                        TBDBG("IP: proc eth pkt: vRelease")
			vReleaseNetworkBufferAndDescriptor( pxNetworkBuffer );
			break;
	}
}
