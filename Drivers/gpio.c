...
{
    unsigned long mask = (1<<(pinNum%32));
    unsigned long clkIndex = pinNum/32;

    pRegs->GPPUD[0] = state;
    //BCM Peripheral Spec says wait 150 cycles, however
    //   I am not sure "the cycles of what" they are referring to
    //   Probably the APB bus, which means the following will
    //   likely be too long? I guess, waitUs is like a 1 Mhz clock.
    waitUs(150);
    pRegs->GPPUDCLK[clkIndex] = mask;
    waitUs(150);

    pRegs->GPPUD[0] = 0;
    pRegs->GPPUDCLK[clkIndex] = 0;
}
